...
def plot_property_distributions(df, save_path):
    """
    Create 3x3 violin plots for key properties.
    ...
    fig, axes = plt.subplots(3, 3, figsize=(10, 8), dpi=600)
    axes = axes.flatten()
    ...
    for idx, (prop, title) in enumerate(viz_properties):
        ...
        ax.set_title(f'({subplot_letter}) {title}', fontweight='bold', loc='left', fontsize=8)
...
def plot_charge_distributions(df, save_path):
    """
    Create 2x2 subplot layout for positive charge fraction probability distributions.
    ...
    for idx, (label, color) in enumerate(zip(DATASET_LABELS, DATASET_COLORS)):
        ...
        # Formatting
        subplot_letter = chr(ord('a') + idx)
        ax.set_title(f'({subplot_letter}) {label}', fontweight='bold', loc='left', fontsize=8)
        ax.set_xlabel('Positive Charge Fraction (K, R)', fontsize=7)
    ...
    print(f"Charge distribution plots saved to: {save_path}")


def plot_correlation_matrices(df, save_path):
    """
    Create 2x2 subplot layout for correlation matrices.
    ...
    for idx, (label, color) in enumerate(zip(DATASET_LABELS, DATASET_COLORS)):
        ...
        if len(dataset_df) > 0:
            ...
            sns.heatmap(correlation_matrix, annot=False, cmap='coolwarm', center=0,
                       square=True, fmt='.2f', cbar=(idx == 3),
                       cbar_ax=cbar_ax if idx == 3 else None, ax=ax)
            ...
            ax.set_xticklabels(acronym_cols, fontsize=7, rotation=45, ha='center')
            ax.set_yticks(tick_positions)
            ax.set_yticklabels(acronym_cols, fontsize=7)
        else:
            ax.set_title(f'{label} - No Data', fontweight='bold', fontsize=8)
            ax.axis('off')
...
def analyze_peptide_properties():
    ...
    summary_path = os.path.join(PATH_RESULT, "amyloid_properties_analysis.csv")
    save_summary_statistics(df, summary_path)
    
    # Create visualizations
    dist_plot_path = os.path.join(PATH_RESULT, "amyloid_properties_distributions.png")
    plot_property_distributions(df, dist_plot_path)
    
    charge_plot_path = os.path.join(PATH_RESULT, "positive_charge_probability_distributions_2x2.png")
    plot_charge_distributions(df, charge_plot_path)
    
    corr_plot_path = os.path.join(PATH_RESULT, "dataset_specific_correlations_2x2.png")
