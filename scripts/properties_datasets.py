...
# Load sequences from the three files (excluding Real AMYs)
path = os.path.expanduser("~/workspace/amyAMP/results/")
fasta_files = ["seqs_generated_postprocessed.fasta", "seqs_realAMPs_realAMYs1000.fasta", "random_peptides_1000.fasta"]
file_labels = ["amyAMP", "trainPep", "randPep"]  # Removed "Real AMYs"
colors = ["#4472C4", "#E15759", "#FFC000"]  # Blue, Red, Yellow
...
for idx, (prop, title) in enumerate(viz_properties):
    ...
    # Create violin plot for all 3 datasets
...
# Create a 1x3 subplot layout for positive charge fraction probability distributions
print("\n" + "="*60)
...
plt.tight_layout()
plt.savefig(path + "positive_charge_probability_distributions.png", dpi=600, bbox_inches='tight')
plt.show()
...
# Create a 1x3 subplot layout for correlation matrices with a single color bar
fig, axes = plt.subplots(1, 3, figsize=(10, 4), gridspec_kw={'width_ratios': [1, 1, 1]})  # Match figure size with distribution plot
...
for idx, (label, color) in enumerate(zip(file_labels, colors)):
    ...
    if len(dataset_df) > 0:
        ...
        sns.heatmap(correlation_matrix, annot=False, cmap='coolwarm', center=0, 
                    square=True, fmt='.2f', cbar=(idx == 2), cbar_ax=cbar_ax if idx == 2 else None, ax=ax)
...
plt.savefig(path + "dataset_specific_correlations.png", dpi=600, bbox_inches='tight')
plt.show()