...
def pca_analysis(embedded_data, group_sizes, group_labels, filename_base, reference_vector=None, nonlinear=False):
    """
    Perform PCA (or KernelPCA) on peptide embeddings and visualize group distributions.
    
    Args:
        embedded_data: np.ndarray of shape (n_samples, n_features)
        group_sizes: list of ints indicating group sizes
        group_labels: list of labels for groups
        filename_base: base name for output file
        reference_vector: optional (1, n_features) array for reference point
        nonlinear: if True, uses KernelPCA with RBF kernel
    """
    ...
    if explained is not None:
        ax1.plot(range(1, len(explained) + 1), np.cumsum(explained), 'bo-')
        ax1.set_xlabel('Number of Components')
        ax1.set_ylabel('Cumulative Explained Variance Ratio')
        total_var = np.sum(explained[:2]) * 100
        print(f"PC1 + PC2 explain {total_var:.2f}% of total variance")
    ...
    for i, (group_size, label) in enumerate(zip(group_sizes, group_labels)):
        ...
        ax2.scatter(group_data[:, 0], group_data[:, 1],
                    color=color, label=label, alpha=0.5, s=8, edgecolors='k', linewidth=0.2)
        start_idx = end_idx

    # Reference marker
    if reference_vector is not None:
        ref_pca = pca.transform(scaler.transform(reference_vector.reshape(1, -1)))
        ax2.scatter(ref_pca[0, 0], ref_pca[0, 1], color='black', s=50, marker='*', label='Reference')
...
if __name__ == "__main__":
    ...
    l_fasta = [path_result+"sequence/"+"seqs_realAMPs"+str(batch_generate)+".fasta",
               ...
               path_data+ "random_peptides_1000_2.fasta"]
